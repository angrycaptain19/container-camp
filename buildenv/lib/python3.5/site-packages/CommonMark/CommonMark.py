    ...
    def prepare(block):
        ...
        for attr in dir(block):
            if (
                ...
                and attr != "makeBlock"
                and block.__dict__[attr] in ["", [], None, {}]
            ):
...
class InlineParser(object):
        ...
        if (numdelims == 1):
            while True:
                ...
                if (res["numdelims"] >= 1 and res["can_close"]):
                    ...
                    if len(inlines) > 1:
                        for _ in range(delimpos + 1, len(inlines)):
            ...
            while True:
                ...
                if (
                    ...
                    and res["numdelims"] <= 3
                    and res["can_close"]
                    and res["numdelims"] != first_close_delims
                ):
    ...
    def parseLinkLabel(self):
        ...
        self.pos += 1
        c = self.peek()
        while (c != "]" or nest_level > 0) and c is not None:
            if c == "`":
                self.parseBackticks([])
            elif c == "<":
    ...
    def parseLink(self, inlines):
        ...
        if normalizeReference(reflabel) in self.refmap:
            ...
            link = None
        if link:
            title = link['title'] if link.get("title", None) else ""
            destination = link['destination'] if link.get("destination", None) else ""
            inlines.append(
...
class DocParser:
    ...
    def endsWithBlankLine(self, block):
        ...
        if block.last_line_blank:
            return True
        if block.t in ["List", "ListItem"] and len(block.children) > 0:
    ...
    def addChild(self, tag, line_number, offset):
        ...
        while (
            self.tip.t != "Document"
            and self.tip.t != "BlockQuote"
            and self.tip.t != "ListItem"
            and (self.tip.t != "List" or tag != "ListItem")
        ):
    ...
    def incorporateLine(self, ln, line_number):
        ...
        def closeUnmatchedBlocks(self, already_done, oldtip):
            ...
            while not already_done and oldtip != last_matched_container:
...
class HTMLRenderer(object):
    ...
    def renderInlines(self, inlines):
        ...
        return ''.join(self.renderInline(inlines[i]) for i in range(len(inlines)))
    ...
    def renderBlocks(self, blocks, in_tight_list=None):
        ...
        result = [
            self.renderBlock(blocks[i], in_tight_list)
